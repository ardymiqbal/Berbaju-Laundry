...
$today = date('Y-m-d');
$isNataru = ($today == date('Y') . '-01-09' || $today == date('Y') . '-01-01');

// Memastikan pop-up hanya muncul sekali
if ($isNataru && empty($_SESSION['nataru_popup_shown'])) {
    $_SESSION['nataru_popup_shown'] = true; // Tandai bahwa pop-up sudah ditampilkan
    $showPopup = true; // Tampilkan pop-up
} else {
    $showPopup = false; // Jangan tampilkan pop-up
}
?>
...
<html>
  ...
  <body class="fixed-left">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const isNataru = <?php echo json_encode($isNataru); ?>;
        const showPopup = <?php echo json_encode($showPopup); ?>;


        // Jika perlu menampilkan popup
        if (isNataru && showPopup) {
            Swal.fire({
              title: '<span style="color: #E63946;">Selamat!</span>',
            html: `
                <div style="font-size: 18px; color: #1D3557; margin-bottom: 10px;">
                    Selamat Tahun Baru atau Selamat Natal! 🎄✨
                </div>
                <div style="color: #457B9D;">
                    Dapatkan diskon <b>20%</b> di layanan kami.
                </div>
                <img src="assets/event/happynewyear1.jpg" alt="Natal dan Tahun Baru" 
                     style="width: 100%; margin-top: 15px; border-radius: 8px;" />
            `,
            icon: 'success',
            confirmButtonText: '<i class="fas fa-music"></i> Putar Musik',
            background: '#F1FAEE',
            customClass: {
            ...
            // Jika popup tidak ditampilkan, musik tetap berjalan
            if (audio.paused) {
                audio.play();
            }
        }
