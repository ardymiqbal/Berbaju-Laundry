...
if (isset($_POST['tambah'])) {
  ...
  if (strlen($userpass) < 8) {
    $pesan_error_pass = "Password minimal 8 karakter <br>";
  }
  ...
  if ($pesan_error_user == "" && $pesan_error_pass == "") {
    ...
    $query = mysqli_query($conn, "INSERT INTO `tb_users`(`userid`, `username`, `userpass`, `nama`, `jk`, `alamat`, `usertelp`, `level`) VALUES (NULL, '$username', '$password', '$nama', '$jk', '$alamat', '$usertelp', '$level')");
    if ($query) {
      echo "
      <script>
