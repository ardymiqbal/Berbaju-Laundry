...
if (isset($_POST['tambah'])) {
  ...
  // Validasi angka
  if ($pengeluaran < 0) {
    $pesan_error = "Nominal pengeluaran tidak boleh negatif!";
  }

  if ($pesan_error === "") {
    // Insert ke tb_pengeluaran
    $query = mysqli_query($conn, 
      "INSERT INTO tb_pengeluaran (id_pengeluaran, tgl_pengeluaran, catatan, pengeluaran) 
       VALUES ('$id_pengeluaran', '$tanggal', '$catatan', '$pengeluaran')"
    );

    // Insert ke tb_laporan
    $query2 = mysqli_query($conn, 
      "INSERT INTO tb_laporan (tgl_laporan, ket_laporan, catatan, id_pengeluaran, pengeluaran)
       VALUES ('$tanggal', '$ket_laporan', '$catatan', '$id_pengeluaran', '$pengeluaran')"
    );
    ...
    if ($query && $query2) {
      ...
      $pesan_error = "Data pengeluaran gagal disimpan: " . mysqli_error($conn);
    }
  }
      ...
      <div class="card m-b-100">
        ...
        <input type="hidden" value="<?= $id_pengeluaran; ?>" name="id_pengeluaran">
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-10">
              <input class="form-control" type="date" name="tanggal" value="<?= $tanggal; ?>" required autofocus/>
            </div>
          </div>
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-10">
              <textarea class="form-control" name="catatan" cols="20" rows="5" placeholder="Masukkan catatan" required><?= $catatan; ?></textarea>
            </div>
          </div>
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-10">
              <input type="number" class="form-control" name="pengeluaran" value="<?= $pengeluaran; ?>" required>
            </div>
          </div>
